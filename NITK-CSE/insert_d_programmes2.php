...
<html>
    ...
    $records=$size;
    /*if($size!=3){
         header("Location: insert_d_programmes.php?uname=" . $username . "&pw=" . $pass);
    }*/
    //echo "<h1> $records </h1><br>";
    for($i=1;$i<=1;$i++){

        $index1="year";
        $index2="no_programmes";
        $index3="no_of_beneficiary";
    ...
    for($i=1;$i<=1;$i++){
        ...
        $sql = "INSERT INTO d_programmes Values($year[$i],'$no_programmes[$i]','$no_of_beneficiary[$i]')";
        echo "";
        if ($mysqli->query($sql) === TRUE) {
                echo "<h1>Record Inserted successfully</h1><br>";
                $link_address="insert_d_programmes.php";
