...
<html>
    <head>
        <meta charset="utf-8" />
        <title>Insert Publicaion Area</title>
    ...
    $records=$size;
    /*if($size!=2){
         header("Location: insert_d_publication_areas.php?uname=" . $username . "&pw=" . $pass);
    }*/
    ...
    for($i=1;$i<=1;$i++){
        //echo "$section[$i] $desp[$i]"; d_publication_areas
        $sql = "INSERT INTO d_publication_areas Values('$area[$i]',$no_of_publication[$i])";

        if ($mysqli->query($sql) === TRUE) {
                echo "<h1>Record Inserted successfully</h1><br>";
                $link_address="insert_d_publication_areas.php";
