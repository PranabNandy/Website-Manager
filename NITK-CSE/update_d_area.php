...
<h2 style="text-align: center;"><a href="login3.php" > ==== > Go To Dashboard Page  < ==== <a></a></h2>
	...
	for($i=1;$i<=$records;$i++){
		...
		$sql = "UPDATE d_area set  full='$full[$i]' WHERE abbrev='$abbrev[$i]' ";
	...
	?>
	 <div class="login">    
<form id="login" action="update_d_area.php" method="POST">
	<table id="tb">
		<tr>
			<th>abbrev</th>
			<th>full</th>
  		...
  		?>
			...
			<tr>
				<td><?php echo "$row[0]"; ?> <input type="hidden" value="<?php echo "$row[0]";?>" 
					name="abbrev<?php echo "$i";?>"/>
				</td>
				<td><input type="text" size="50" value="<?php echo "$row[1]";?>" name="full<?php echo "$i";?>"/>
				</td>
