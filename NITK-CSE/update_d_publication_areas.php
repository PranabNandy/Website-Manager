...
<html>
<head>
		<title>Update Publication Area</title>
</head>
...
<h2 style="text-align: center;"><a href="login3.php" > ==== > Go To Dashboard Page  < ==== <a></a></h2>
	...
	for($i=1;$i<=$records;$i++){
		...
		$sql = "UPDATE d_publication_areas set  no_of_publication='$no_of_publication[$i]' WHERE area='$area[$i]' ";
	...
	$sql = "SELECT * FROM d_publication_areas";
	
	$result = $mysqli -> query($sql) 

	?>
	 <div class="login">    
<form id="login" action="update_d_publication_areas.php" method="POST">
	<table id="tb">
		<tr>
			<th>area</th>
		...
		$i=1;
  		...
  		?>
			...
			<tr>
				<td><?php echo "$row[0]"; ?> <input type="hidden" value="<?php echo "$row[0]";?>" 
					name="area<?php echo "$i";?>"/>
				</td>
				<td><input type="text" size="50" value="<?php echo "$row[1]";?>" name="no_of_publication<?php echo "$i";?>"/>
				</td>
