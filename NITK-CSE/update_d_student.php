...
<html>
<head>
		<title>Update Student Award</title>
</head>
...
<h2 style="text-align: center;"><a href="login3.php" > ==== > Go To Dashboard Page  < ==== <a></a></h2>
	...
	for($i=1;$i<=$records;$i++){
		...
		$sql = "UPDATE d_student set  Award='$Award[$i]' WHERE S_NO=$S_NO[$i] ";
	...
	$sql = "SELECT * FROM d_student";
	
	$result = $mysqli -> query($sql) 

	?>
	 <div class="login">    
<form id="login" action="update_d_student.php" method="POST">
	<table id="tb">
		<tr>
			<th>S_NO</th>
			<th>Award</th>
		...
		$i=1;
  		...
  		?>
			...
			<tr>
				<td><?php echo "$row[0]"; ?> <input type="hidden" value="<?php echo "$row[0]";?>" name="S_NO<?php echo "$i";?>"/>
				</td>
				<td><input type="text" size="50" value="<?php echo "$row[1]";?>" name="Award<?php echo "$i";?>"/>
				</td>
